...
class SchBridgeDiffusion():
    ...
    def cleanup(self):
        """Clean up CUDA tensors to prevent memory leaks"""
        self.betas = None
        self.std_fwd = None
        self.std_bwd = None
        self.std_sb = None
    ...
    def q_sample(self, step, x0, x1, ot_ode=False):
        ...
        # print(f"q_sample: {x0.shape=}, {x1.shape=}, {step=}")
    ...
    def find_std_sb(self, step):
        return self.std_sb[step]

    def bridge_score(self, step, xt, x0, x1):
        assert x0.shape == x1.shape
        batch, *xdim = x0.shape

        mu_x0  = unsqueeze_xdim(self.mu_x0[step],  xdim)
        mu_x1  = unsqueeze_xdim(self.mu_x1[step],  xdim)
        std_fwd_t = self.get_std_fwd(step, xdim)

        bridge_score = - (xt - x0) / std_fwd_t**2
        return bridge_score
    
    def bridge_noise(self, step, xt, x0, x1):
        assert x0.shape == x1.shape
        batch, *xdim = x0.shape
        ...
        mean = mu_x0 * x0 + mu_x1 * x1
        bridge_noise = (xt - x0) / std_fwd_t # not var!!
        return bridge_noise
