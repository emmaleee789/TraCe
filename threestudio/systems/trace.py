...
@threestudio.register("trace-system")
class Trace(BaseLift3DSystem):
    @dataclass
    ...
    def load_custom_pointcloud(self, filepath):
        # Load the pre-generated point cloud
        ...
        # You might need to center or scale the point cloud like in shape()
        # Assuming similar scale as the shape() method
        return coords, rgb, 0.4
    ...
    def pcb(self):
        ...
        elif self.load_type==2:  # New option for custom point cloud
            coords,rgb,scale = self.load_custom_pointcloud(self.load_path)
        else:
    ...
    def on_fit_start(self) -> None:
        ...
        #     self.cfg.prompt_processor
        # )
        # self.guidance = threestudio.find(self.cfg.guidance_type_1)(self.cfg.guidance)
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.true_global_step == self.guidance.cfg.stage_two_start_step:
            if self.guidance.cfg.use_sch_bridge:
                threestudio.info(f"Moving to stage 2 at step {self.true_global_step}")
                self.guidance.phase_id = 2
            elif self.guidance.cfg.use_bridge:
                threestudio.info(f"Moving to stage 2 at step {self.true_global_step}")
                self.guidance.phase_id = 2
                self.prompt_processor = threestudio.find(self.cfg.prompt_processor_type_2)(
                    self.cfg.prompt_processor
                )
        ...
        guidance_out = self.guidance(
            self.true_global_step, images, prompt_utils, **batch, rgb_as_latents=False, guidance_eval=guidance_eval
        )

        loss = 0.0

        loss = loss + guidance_out['loss_sds'] *self.C(self.cfg.loss['lambda_sds']) + guidance_out['loss_lora'] * self.C(self.cfg.loss['lambda_lora']) # optimize in one backward pass and update different parameters

        self.log("train/loss_sds", guidance_out['loss_sds'], prog_bar=True, logger=True, on_step=True, on_epoch=False) # Log SDS loss
        self.log("train/loss_lora", guidance_out['loss_lora'], prog_bar=True, logger=True, on_step=True, on_epoch=False) # Log LoRA loss
        # self.log("train/noise_pred_est", guidance_out['noise_pred_est'], prog_bar=True, logger=True, on_step=True, on_epoch=False) # Log LoRA loss
        # self.log("train/bridge_noise", guidance_out['bridge_noise'], prog_bar=True, logger=True, on_step=True, on_epoch=False) # Log LoRA loss
    ...
    def configure_optimizers(self):
        ...
        # gaussian_params = self.gaussian.params_list
        # lora_params = self.guidance.lora_parameters
        # params = gaussian_params
        # params.append({"params": lora_params, "lr": self.guidance.cfg.lora_learning_rate})

        # optimizer = torch.optim.Adam(
        #     params, lr=0.0, eps=self.cfg.eps # Set base lr to 0.0 since each param group has its own lr
        # )
        
        # Optimizer for Gaussian parameters
        optimizer_gauss = self.gaussian.optimizer

        # Optimizer for LoRA parameters
        optimizer_lora = torch.optim.Adam(
            self.guidance.lora_parameters, lr=self.guidance.cfg.lora_learning_rate
        )

        ret = {
            "optimizer": optimizer_gauss,
